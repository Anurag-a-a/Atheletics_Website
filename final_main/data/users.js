...
export const getUserbyId = async (id) => {
  ...
  if (theUser === null) { throw "No band with that id"; }; // error comment does not make sense 
...
export const getAllUser = async () => {
  ...
  if (!userList) { throw "Could not get all dogs"; };//users
...
export const updateAppointment = async (
  ...
  if (action == 'delete') {
    for (let i = 0; i < CurrentAppointmentList.length; i++) {
      if (CurrentAppointmentList[i] != MyAppointmentsId) {
        newAppointmentList.push(CurrentAppointmentList[i]);
      };
    };
